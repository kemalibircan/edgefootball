import type {MathCouponItem} from '../../types/api';
...
export type MathCouponVariant = keyof typeof STRATEGY_PROFILES;

export type DecisionContext = {
  strategyKey?: string;
  targetRange?: {min?: number; max?: number} | null;
  targetRangeByVariant?: Partial<Record<MathCouponVariant, {min?: number; max?: number} | null>>;
};

export type MathCouponDecisionItem = MathCouponItem & {
  decision: 'play' | 'skip';
  score: number;
  reasons: string[];
  variant: MathCouponVariant;
  threshold: number;
};
...
export function resolveCouponVariant(couponId: unknown, strategyKey: unknown): MathCouponVariant {
  const strategy = String(strategyKey || '').trim();
  if (strategy && strategy !== 'mix_portfolio' && strategy in STRATEGY_PROFILES) {
    return strategy as MathCouponVariant;
  }
  const normalizedId = String(couponId || '')
    .trim()
    .toLowerCase();
...
export function computeOddsFitScore(totalOdds: unknown, targetRange: {min?: number; max?: number} | null | undefined) {
...
export function computeCouponDecision(item: MathCouponItem, context: DecisionContext = {}) {
  ...
  const reasons = [`Edge seviyesi: ${edgeLevel}.`, `Oran durumu: ${oddsReason}.`];
  if (variant === 'mix_shot') {
...
export function groupCouponsByDecision(items: MathCouponItem[] = [], context: DecisionContext = {}) {
  const normalized = Array.isArray(items) ? items : [];
  const withDecision: MathCouponDecisionItem[] = normalized.map(item => {
